...
const Rewards = ({ userId }) => {
  ...
  const claimDailyReward = async () => {
    setIsLoading(true);
    try {
      const response = await axios.post('https://xp-earner.onrender.com/api/rewards/claim-daily-reward', { userId });
  ...
  const claim12HourReward = async () => {
    setIsLoading(true);
    try {
      const response = await axios.post('https://xp-earner.onrender.com/api/rewards/claim-12hour-reward', { userId });
      if (response.data.success) {
        alert(`You have received ${response.data.points} points!`);
